...
func (h* ESReader) process(json []byte, default_size int) (string, int32, error) {
	...
	jsonparser.EachKey(json, func(idx int, value []byte, vt jsonparser.ValueType, err error) {
		...
		if idx == 0 {
			// _scroll_id
			h.scroll_id = string(value)
			plugin.DbgLog("SCROLL_ID %s", h.scroll_id)
