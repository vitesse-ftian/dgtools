...
select
	n_name,
	sum(l_extendedprice * (1 - l_discount))::numeric(18,2) as revenue
from
	customer,
	orders,
	lineitem,
...
create view q6 as 
select
	sum(l_extendedprice * l_discount)::numeric(18,2) as revenue
from
	lineitem
where
...
from
	(
		...
		where
			...
			-- CK: much faster if we add these 2 conds. PG planner is not too smart.
			-- and (n1.n_name = 'FRANCE' or n1.n_name = 'ARGENTINA')
			-- and (n2.n_name = 'FRANCE' or n2.n_name = 'ARGENTINA')
...
where
	...
	and (l_shipmode = 'FOB' or l_shipmode = 'TRUCK') /*l_shipmode in ('FOB', 'TRUCK')*/
	and l_commitdate < l_receiptdate
...
select
	...
	end)::numeric(18,2) / sum(l_extendedprice * (1 - l_discount))::numeric(18,2) as promo_revenue
from
	lineitem,
	part
where
...
select
	...
	p_type,
	p_size,
	count(uniq_ps_suppkey) as supplier_cnt
from 
(
select 
	p_brand, p_type, p_size, ps_suppkey as uniq_ps_suppkey
from
	partsupp 
	join part on (
		p_partkey = ps_partkey
	...
	left outer join (select s_suppkey from supplier 
		  where s_comment like '%Customer%Complaints%'
		) supp on (ps_suppkey = s_suppkey )
where
	s_suppkey is null
group by
	p_brand,
...
from
	lineitem,
	part, 
	(select l_partkey as f_partkey, 0.2::double precision * avg(l_quantity) as f_qnt
	 from lineitem 
	 group by 1) foo
where
	p_partkey = l_partkey
...
pstab as (
	select ps_suppkey
	from partsupp, ltab, ptab
	where ps_partkey = ptab.p_partkey
	      and ltab.l_partkey = ps_partkey and ltab.l_suppkey = ps_suppkey
	      and ps_availqty > ltab.s
)
select s_name, s_address
from supplier, nation, pstab
where s_nationkey = n_nationkey and n_name = 'VIETNAM'
      and s_suppkey = pstab.ps_suppkey
;
...
create view q22 as
with cust_with_orders as
(select o_custkey from orders group by 1)
,       
cust as 
(select * from (select c_acctbal, c_phone, c_custkey, substring(c_phone from 1 for 2)::int as cntrycode
from customer) foo left join cust_with_orders on o_custkey = c_custkey where
cntrycode in ('10', '11', '26', '22', '19', '20', '27')
...
drop view if exists qv15_revenue0 cascade;
create view qv15_revenue0 (supplier_no, total_revenue) as
	select
		l_suppkey,
		sum(l_extendedprice * (1 - l_discount))::numeric(18,2)
	from
