...
select
	...
	sum(l_extendedprice)::number(18,2) as sum_base_price,
	sum(l_extendedprice * (1 - l_discount))::number(18,2) as sum_disc_price,
	sum(l_extendedprice * (1 - l_discount) * (1 + l_tax))::number(18,2) as sum_charge,
	avg(l_quantity)::number(18,2) as avg_qty,
	avg(l_extendedprice)::number(18,2) as avg_price,
	avg(l_discount)::number(18,2) as avg_disc,
...
select
	...
	end) / sum(volume))::number(10,5) as mkt_share
from
	(
		select
...
select
	nation,
	o_year,
	sum(amount)::number(18,2) as sum_profit
from
	(
		select
			n_name as nation,
...
select
	...
	end) / sum(l_extendedprice * (1 - l_discount))::number(18,2) as promo_revenue
from
	lineitem,
	part
where
...
create view q15_revenue0 (supplier_no, total_revenue) as
	select
		l_suppkey,
		sum(l_extendedprice * (1 - l_discount))::number(18,2)
	from
		lineitem
	where
...
select
	cntrycode,
	count(*) as numcust,
	sum(c_acctbal)::number(18,2) as totacctbal
from
	(
		select
